...
namespace CardboardMonkey
{
	...
	public class MainActivity : CardboardActivity, CardboardView.IStereoRenderer
	{
		...
		protected override void OnCreate (Bundle savedInstanceState)
		{
			...
			mVibrator = GetSystemService(Context.VibratorService).JavaCast<Vibrator>();
		...
		public void OnDrawEye (EyeTransform transform)
		{
			...
			Matrix.MultiplyMM(view, 0, transform.EyeView, 0, camera, 0);
			...
			Matrix.MultiplyMM(modelViewProjection, 0, transform.Perspective, 0, modelView, 0);
			DrawCube();

			// Set mModelView for the floor, so we draw floor in the correct location
			Matrix.MultiplyMM(modelView, 0, view, 0, modelFloor, 0);
			Matrix.MultiplyMM(modelViewProjection, 0, transform.Perspective, 0,
			              modelView, 0);
			DrawFloor(transform.Perspective);
		}
