...
namespace Google.Cardboard.Sensors.Internal
{
	...
	{
		// TODO: private static readonly float NS2S = 1.0E-0x09F;
...

		{
			...
			mP.setZero();
			mP.SameDiagonal = 25.0D;


			double initialSigmaQ = 1.0D;
			mQ.setZero();
			mQ.SameDiagonal = 1.0D;


			double initialSigmaR = 0.25D;
			mR.setZero();
			mR.SameDiagonal = 0.0625D;


			double initialSigmaRaccel = 0.75D;
			mRaccel.setZero();
			mRaccel.SameDiagonal = 0.5625D;
		...
		{
			get { return sensorTimeStampAcc != 0L; }
		}
		...
		{
			get { return glMatrixFromSo3(so3SensorFromWorld); }
		}
...

		{
			...
			double tmp62_61 = (rotationMatrix[11] = 0.0D);
			rotationMatrix[7] = tmp62_61;
			rotationMatrix[3] = tmp62_61;
			double tmp86_85 = (rotationMatrix[14] = 0.0D);

			rotationMatrix[13] = tmp86_85;
			rotationMatrix[12] = tmp86_85;
...

		{
			lock (this)
			{
				...
				{
					float dT = (float) ((sensorTimeStamp - sensorTimeStampGyro)*Math.Pow(10, -9));
					if (dT > 0.04F)
					{
		...
		{
			Debug.Assert(3 == data.Length);
			Debug.Assert(3 == data[0].Length);
			Debug.Assert(3 == data[1].Length);
			Debug.Assert(3 == data[2].Length);
